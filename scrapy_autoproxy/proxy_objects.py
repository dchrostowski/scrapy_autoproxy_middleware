...
DECREMENT_BLACKLIST = config.settings['decrement_blacklist']
...
class ProxyObject(Proxy):
    ...
    def callback(self, success):
        logger.info("callback=%s for proxy %s" % (success,self.urlify()))
        ...
        if success is None:
            ...
            logger.info("proxy.callback(success=True)")
            ...
            logger.info("proxy.callback(success=False)")
